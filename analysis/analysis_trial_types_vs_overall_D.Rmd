...
subtitle: "Comparing IRAP *D* scoring by trial type vs sigle overall score"
author: "Ian Hussey"
...
data_estimates_overall_D <- read_csv("../data/processed/data_estimates_overall_D.csv") %>%
  mutate(trial_type = "overall")
...
data_map_ci_widths <- data_estimates_overall_D %>%
...
data_ci_width_map_D <- data_estimates_overall_D %>%
  group_by(domain, trial_type) %>%
  ...
  mutate(MAP = round_half_up(MAP, 3),
         trial_type = case_when(trial_type == "tt1" ~ "Trial type 1",
                                ...
                                trial_type == "overall" ~ "Overall"),
         trial_type = fct_relevel(trial_type, "Trial type 1", "Trial type 2", "Trial type 3", "Trial type 4", "Overall")) %>%
...
data_diff_zero <- 
  bind_rows(mutate(data_estimates_D, splitting_method = "Trial types"),
            mutate(data_estimates_overall_D, splitting_method = "Overall")) %>%
  mutate(domain = as.factor(domain),
         trial_type = case_when(trial_type == "tt1" ~ "Trial type 1",
                                ...
                                trial_type == "overall" ~ "Overall"),
         trial_type = fct_relevel(trial_type, "Trial type 1", "Trial type 2", "Trial type 3", "Trial type 4", "Overall")) %>%
  group_by(domain, trial_type, splitting_method) %>%
...
p_diff_zero <- 
  ...
  ggplot(aes(proportion_diff_zero, domain, color = splitting_method, shape = splitting_method)) +
  ...
  scale_shape_manual(labels = c("Trial types", "Overall"), values = c(15, 16)) +
  scale_color_viridis_d(begin = 0.3, end = 0.7, labels = c("Trial types", "Overall")) +
  mdthemes::md_theme_linedraw() +
...
# fit model
fit_diff_zero <- 
  lmer(proportion_diff_zero_logit ~ 1 + splitting_method + (1 | domain/trial_type),
       weights = 1/variance, 
       data = data_diff_zero)

# extract marginal means
results_emm_diff_zero <- 
  summary(emmeans(fit_diff_zero, ~ splitting_method)) %>%
  dplyr::select(splitting_method, estimate = emmean, se = SE, ci_lower = lower.CL, ci_upper = upper.CL)
...
# plot
p_prop_nonzero <- 
  ggplot(results_diff_zero, aes(splitting_method, estimate, 
                                #color = splitting_method, 
                                #shape = splitting_method, 
                                #group = splitting_method
  )) +
  ...
  #scale_color_viridis_d(alpha = 1, begin = 0.3, end = 0.7, labels = c("Trial types", "Overall")) +
  #scale_shape_manual(labels = c("Trial types", "Overall"), values = c(15, 16)) +
  labs(x = "",
...
# tests
data_emms_diff_zero <- emmeans(fit_diff_zero, list(pairwise ~ splitting_method), adjust = "holm") 

summary(data_emms_diff_zero)$`pairwise differences of splitting_method` %>%
  as.data.frame() %>%
...
if(file.exists("../data/results/data_discriminability_overall_D.csv")) {
  
  data_discriminability_overall_D <- read_csv("../data/results/data_discriminability_overall_D.csv") %>%
    filter(method == "bca")
  
} else {
  
  # bootstrap D scores 
  data_discriminability_overall_D <- data_estimates_overall_D |>
    ...
    mutate(variance = ((ci_upper - ci_lower)/(1.96*2))^2,
           ...
           trial_type = fct_relevel(trial_type, "tt1", "tt2", "tt3", "tt4", "overall"),
           splitting_method = "Overall") 
  
  # save to disk
  write_csv(data_discriminability_overall_D, "../data/results/data_discriminability_overall_D.csv")
...
data_discriminability_combined <- 
  bind_rows(data_discriminability_D,
            data_discriminability_overall_D) %>%
...
p_discriminability <- 
  data_discriminability_combined %>%
  mutate(domain = fct_rev(factor(domain))) %>%
  ggplot(aes(proportion_discriminable, domain, color = splitting_method, shape = splitting_method)) +
...
fit_disciminability <- 
  lmer(proportion_discriminable_logit ~ 1 + splitting_method + (1 | domain/trial_type), 
       weights = 1/variance, 
       data = data_discriminability_combined)

# extract marginal means
results_emm_disciminability <-
  summary(emmeans(fit_disciminability, ~ splitting_method)) %>%
  dplyr::select(splitting_method, estimate = emmean, se = SE, ci_lower = lower.CL, ci_upper = upper.CL) 
...
p_prop_discriminable <-
  ggplot(results_disciminability, aes(splitting_method, estimate, 
                                      #color = splitting_method, 
                                      #shape = splitting_method, 
                                      #group = splitting_method
  )) +
...
# tests
data_emms_disciminability <- emmeans(fit_disciminability, list(pairwise ~ splitting_method), adjust = "holm") 

summary(data_emms_disciminability)$`pairwise differences of splitting_method` %>%
  as.data.frame() %>%
  select(comparison = 1, p.value) %>%
  mutate(p.value = ifelse(p.value < .001, "< .001", round_half_up(p.value, 3))) %>%
  kable() %>%
...
observed_range_estimates_overall_D <- data_estimates_overall_D %>%
  group_by(domain, trial_type) %>%
  dplyr::summarize(min = min(ci_lower, na.rm = TRUE),
                   max = max(ci_upper, na.rm = TRUE),
                   .groups = "drop") %>%
...
data_ci_width_proportions_D <- data_estimates_D %>%
  ...
  mutate(splitting_method = "Trial types") 

data_ci_width_proportions_overall_D <- data_estimates_overall_D %>%
  # join this data into the original data
  full_join(observed_range_estimates_overall_D, by = c("domain", "trial_type")) %>%
  # calculate ci width as a proportion of observed range
  mutate(ci_width_proportion = ci_width / range) %>%
  mutate(splitting_method = "Overall")

# combine
data_ci_width_proportions_combined <- 
  bind_rows(data_ci_width_proportions_D,
            data_ci_width_proportions_overall_D) %>%
  mutate(domain = as.factor(domain),
         trial_type = fct_relevel(trial_type, "tt1", "tt2", "tt3", "tt4", "overall")) %>%
  # logit transform
...
fit_ci_width_proportions <- 
  lmer(ci_width_proportion_logit ~ 1 + splitting_method + (1 | domain/trial_type), 
       data = data_ci_width_proportions_combined)

# extract marginal means
results_emm_ci_width_proportions <-
  summary(emmeans(fit_ci_width_proportions, ~ splitting_method)) %>%
  dplyr::select(splitting_method, estimate = emmean, se = SE, ci_lower = asymp.LCL, ci_upper = asymp.UCL)
...
p_ci_width_proportion_observed_range <-
  ggplot(results_ci_width_proportions, aes(splitting_method, estimate, 
                                           #color = splitting_method, 
                                           #shape = splitting_method, 
                                           #group = splitting_method
  )) +
  ...
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), size = 1.25, position = position_dodge(width = 0.8)) +
  geom_point(position = position_dodge(width = 0.8), size = 2.5) +
  scale_shape_discrete(labels = c("Trial types", "Overall")) +
...
# tests
data_emms_ci_width_proportions <- emmeans(fit_ci_width_proportions, list(pairwise ~ splitting_method), adjust = "holm") 

summary(data_emms_ci_width_proportions)$`pairwise differences of splitting_method` %>%
  as.data.frame() %>%
  select(comparison = 1, p.value) %>%
  mutate(p.value = ifelse(p.value < .001, "< .001", round_half_up(p.value, 3))) %>%
  kable() %>%
...
ggsave(filename  = "plots/figure_1_cis_by_domain_irap_overall.pdf",
...
ggsave(filename  = "plots/figure_2_ci_widths_irap_overall.pdf",
       plot      = p_ci_widths,
       device    = "pdf",
       # path      = NULL,
       # dpi       = 300,
...
ggsave(filename  = "plots/figure_3_metaanalyses_irap_overall.pdf",
       plot      = p_combined,
       device    = "pdf",
       # path      = NULL,
       # dpi       = 300,
