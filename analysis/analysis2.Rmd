...
data_diff_zero <- 
  ...
  mutate(variance = ifelse(variance == 0, 0.0001, variance),
         proportion_diff_zero_temp = case_when(proportion_diff_zero < 0.001 ~ 0.001, 
...
results_diff_zero_scoring <- results_emm_diff_zero_scoring %>%
  mutate(pi_lower = estimate - (1.96 * sqrt(se^2 + results_re_tau_diff_zero_scoring$tau[2]^2)),  # as in metafor package's implementation of prediction intervals, see metafor::predict.rma.R 
         pi_upper = estimate + (1.96 * sqrt(se^2 + results_re_tau_diff_zero_scoring$tau[2]^2))) |>
  select(-se) |>
...
results_disciminability_scoring <- results_emm_disciminability_scoring %>%
  mutate(pi_lower = estimate - (1.96 * sqrt(se^2 + results_re_tau_disciminability_scoring$tau[2]^2)),  # as in metafor package's implementation of credibility intervals, see metafor::predict.rma.R
         pi_upper = estimate + (1.96 * sqrt(se^2 + results_re_tau_disciminability_scoring$tau[2]^2))) |>
  select(-se) |>
  mutate_if(is.numeric, boot::inv.logit)
...
results_ci_width_proportions_scoring <- results_emm_ci_width_proportions_scoring %>%
  mutate(pi_lower = estimate - (1.96 * sqrt(se^2 + results_re_tau_ci_width_proportions_scoring$tau[2]^2)),  # as in metafor package's implementation of credibility intervals, see metafor::predict.rma.R
         pi_upper = estimate + (1.96 * sqrt(se^2 + results_re_tau_ci_width_proportions_scoring$tau[2]^2))) %>%
  select(-se) %>%
  mutate_if(is.numeric, boot::inv.logit)
