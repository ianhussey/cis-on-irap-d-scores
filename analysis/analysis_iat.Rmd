...
# fit model
fit_diff_zero <- 
  lmer(proportion_diff_zero_logit ~ 1 + (1 | domain),
       weights = 1/variance, 
       data = data_diff_zero,
...
results_emm_diff_zero <- 
  summary(emmeans(fit_diff_zero, ~ 1)) %>%
  dplyr::select(estimate = emmean, se = SE, ci_lower = lower.CL, ci_upper = upper.CL)
...
# plot
p_prop_nonzero <- 
  ggplot(results_diff_zero, aes("IAT D score", estimate, 
  )) +
...
p_discriminability <- 
  data_discriminability_combined %>%
  mutate(domain = fct_rev(factor(domain))) %>%
  ggplot(aes(proportion_discriminable, domain, color = measure, shape = measure)) +
...
fit_disciminability <- 
  lmer(proportion_discriminable_logit ~ 1 + (1 | domain), 
       weights = 1/variance, 
...
results_emm_disciminability <-
  summary(emmeans(fit_disciminability, ~ 1)) %>%
...
p_prop_discriminable <-
  ggplot(results_disciminability, aes("IAT D score", estimate, 
  )) +
  geom_linerange(aes(ymin = pi_lower, ymax = pi_upper), size = 0.5, position = position_dodge(width = 0.8), linetype = "dotted") +
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), size = 1.25, position = position_dodge(width = 0.8)) +
  geom_point(position = position_dodge(width = 0.8), size = 2.5) +
...
fit_ci_width_proportions <- 
  lmer(ci_width_proportion_logit ~ 1 + (1 | domain), 
...
results_emm_ci_width_proportions <-
  summary(emmeans(fit_ci_width_proportions, ~ 1)) %>%
  dplyr::select(estimate = emmean, se = SE, ci_lower = asymp.LCL, ci_upper = asymp.UCL)
...
p_ci_width_proportion_observed_range <-
  ggplot(results_ci_width_proportions, aes("IAT D score", estimate, 
  )) +
  geom_linerange(aes(ymin = pi_lower, ymax = pi_upper), size = 0.5, position = position_dodge(width = 0.8), linetype = "dotted") +
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), size = 1.25, position = position_dodge(width = 0.8)) +
  geom_point(position = position_dodge(width = 0.8), size = 2.5) +
...
ggsave(filename  = "plots/figure_1_cis_by_domain_iat.pdf",
...
ggsave(filename  = "plots/figure_2_ci_widths_iat.pdf",
       plot      = p_ci_widths,
       device    = "pdf",
       # path      = NULL,
       # dpi       = 300,
...
ggsave(filename  = "plots/figure_3_metaanalyses_iat.pdf",
       plot      = p_combined,
       device    = "pdf",
       # path      = NULL,
       # dpi       = 300,
