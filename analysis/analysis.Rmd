...
bootstrap_discriminability <- function(data){
  ...
  results <- boot::boot.ci(fit, conf = 0.95, type = "norm")

  output <-
    tibble(
      estimate = fit$t0,
      ci_lower = results$normal[2],
      ci_upper = results$normal[3]
    )
...
results_emm_disciminability_method <-
  summary(emmeans(fit_disciminability_method, ~ method)) %>%
  dplyr::select(method, estimate = emmean, se = SE, ci_lower = lower.CL, ci_upper = upper.CL) 

# combine
results_disciminability_method <- results_emm_disciminability_method %>%
  mutate(pi_lower = estimate - (1.96 * sqrt(se^2 + sum(results_re_tau_disciminability_method$tau)^2)),  # as in metafor package's implementation of credibility intervals, see metafor::predict.rma.R
         pi_upper = estimate + (1.96 * sqrt(se^2 + sum(results_re_tau_disciminability_method$tau)^2)))

# plot
p_prop_discriminable_method <-
  ggplot(results_disciminability_method, aes(method, estimate, color = method, shape = method, group = method)) +
  geom_linerange(aes(ymin = pi_lower, ymax = pi_upper), position = position_dodge(width = 0.8), linetype = "dotted") +
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), size = 1, position = position_dodge(width = 0.8)) +
  geom_point(position = position_dodge(width = 0.8), size = 2.5) +
  scale_y_continuous(breaks = c(0, .25, .5, .75, 1), labels = c("0.00<br/>(Worse)", "0.25", "0.50", "0.75", "1.00<br/>(Better)")) +
...
results_disciminability_method %>%
  round_df(2) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
