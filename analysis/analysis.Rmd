...
results_emm_diff_zero_method <- 
  summary(emmeans(fit_diff_zero_method, ~ DV_type | trial_type)) %>%
  select(DV_type, trial_type, estimate = emmean, ci_lower = lower.CL, ci_upper = upper.CL) %>%
  mutate(trial_type = fct_relevel(trial_type, "tt1", "tt2", "tt3", "tt4"))

p_diff_zero_method <- 
  ggplot(results_emm_diff_zero_method, aes(trial_type, estimate, color = DV_type, shape = DV_type, group = DV_type)) +
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = dodge_width)) +
  geom_line(position = position_dodge(width = dodge_width)) +
  geom_point(position = position_dodge(width = dodge_width), size = 2.5) +
  mdthemes::md_theme_linedraw() +
...
# tests
emmeans(fit_diff_zero_method, list(pairwise ~ DV_type | trial_type), adjust = "bonferroni")
emmeans(fit_diff_zero_method, list(pairwise ~ trial_type | DV_type), adjust = "bonferroni")
