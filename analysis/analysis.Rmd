...
- reorder and relabel boot methods for all plots
- prop diff from zero labs(color = "Bootstrapping method") + , add coloring
...
# save to disk
write_rds(results_emm_diff_zero_scoring, "models/results_emm_diff_zero_scoring.rds")

ggplot(results_emm_diff_zero_scoring, aes(trial_type, estimate, color = DV_type, shape = DV_type, group = DV_type)) +
...
results_emm_diff_zero_method <- 
  ...
  select(method, trial_type, estimate = emmean, ci_lower = lower.CL, ci_upper = upper.CL) %>%
  mutate(trial_type = fct_relevel(trial_type, "tt1", "tt2", "tt3", "tt4"))

# save to disk
write_rds(results_emm_diff_zero_method, "models/results_emm_diff_zero_method.rds")

ggplot(results_emm_diff_zero_method, aes(trial_type, estimate, color = method, shape = method, group = method)) +
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = dodge_width)) +
  geom_line(position = position_dodge(width = dodge_width)) +
  geom_point(position = position_dodge(width = dodge_width), size = 2.5) +
  mdthemes::md_theme_linedraw() +
...
fit_disciminability_scoring <- 
  lmer(proportion_discriminable ~ 1 + trial_type * DV_type + (1 | domain), 
       weights = 1/variance, 
...
ggplot(results_emm_disciminability_scoring, aes(trial_type, estimate, color = DV_type, shape = DV_type, group = DV_type)) +
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = dodge_width)) +
  geom_line(position = position_dodge(width = dodge_width)) +
  geom_point(position = position_dodge(width = dodge_width), size = 2.5) +
...
# save to disk
write_rds(results_emm_disciminability_scoring, "models/results_emm_disciminability_scoring.rds")
...
fit_disciminability_method <- 
  lmer(proportion_discriminable ~ 1 + trial_type * method + (1 | domain), 
       weights = 1/variance, 
...
# save to disk
write_rds(results_emm_disciminability_method, "models/results_emm_disciminability_method.rds")
...
# save to disk
write_rds(results_emm_ci_width_proportions_scoring, "models/results_emm_ci_width_proportions_scoring.rds")
...
# save to disk
write_rds(results_emm_ci_width_proportions_method, "models/results_emm_ci_width_proportions_method.rds")
