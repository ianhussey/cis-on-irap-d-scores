...
# bootstrapping has a long execution time, so load saved values if they've already been calculated
if(file.exists("models/data_discriminability_rpb.rds")) {
  
  data_discriminability_rpb <- read_rds("models/data_discriminability_rpb.rds")
  
} else {
  
  # bootstrap D scores 
  data_discriminability_rpb <- data_estimates_rpb %>%
    select(unique_id, domain, trial_type, estimate, ci_upper, ci_lower) %>%
    group_by(domain, trial_type) %>%
    do(bootstrap_discriminability(data = .)) %>%
    ungroup() %>%
  ...
  # save to disk
  write_rds(data_discriminability_rpb, "models/data_discriminability_rpb.rds", compress = "gz")
...
# combine
data_discriminability_combined_method <- 
  bind_rows(data_discriminability_D,
            data_discriminability_rpb)

# fit meta analytic model
fit_disciminability_method <- 
  lmer(proportion_discriminable ~ 1 + trial_type * DV_type + (1 | domain), 
       weights = 1/variance, 
       data = data_discriminability_combined_method)
...
results_emm_disciminability_method <- 
  summary(emmeans(fit_disciminability_method, ~ DV_type | trial_type)) %>%
  select(DV_type, trial_type, estimate = emmean, ci_lower = lower.CL, ci_upper = upper.CL) %>%
  mutate(trial_type = fct_relevel(trial_type, "tt1", "tt2", "tt3", "tt4"))
  
p_disciminability_method <- 
  ggplot(results_emm_disciminability_method, aes(trial_type, estimate, color = DV_type, shape = DV_type, group = DV_type)) +
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = dodge_width)) +
  geom_line(position = position_dodge(width = dodge_width)) +
  geom_point(position = position_dodge(width = dodge_width), size = 2.5) +
  scale_shape_discrete() +
...
# tests
emmeans(fit_disciminability_method, list(pairwise ~ DV_type | trial_type), adjust = "bonferroni")
emmeans(fit_disciminability_method, list(pairwise ~ trial_type | DV_type), adjust = "bonferroni")
