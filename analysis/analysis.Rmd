...
data_trial_level <- read_csv("../data/data_trial_level.csv") %>%
  filter(timepoint == "baseline" & age >= 18)
...
Calculated by trial-type

```{r}

# bootstrapping has a long execution time, so load saved values if they've already been calculated
if(file.exists("../data/data_estimates_D.rds")) {
  
  data_estimates_D <- read_rds("../data/data_estimates_D.rds")
  ...
  # save to disk
  write_rds(data_estimates_D, "../data/data_estimates_D.rds", compress = "gz")
...
# bootstrapping has a long execution time, so load saved values if they've already been calculated
if(file.exists("../data/data_estimates_PI.rds")) {
  
  data_estimates_PI <- read_rds("../data/data_estimates_PI.rds")
  ...
  # save to disk
  write_rds(data_estimates_PI, "../data/data_estimates_PI.rds", compress = "gz")
...
p_prop_nonzero_method <- 
  ...
  geom_linerange(aes(ymin = pi_lower, ymax = pi_upper), position = position_dodge(width = dodge_width), linetype = "dotted") +
