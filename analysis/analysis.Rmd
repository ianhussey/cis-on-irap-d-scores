...
data_for_analysis <- read_csv("../data/data_internalconsistency_cleaned_trial_level.csv") 
...
if(file.exists("models/data_estimates_with_CIs.Rdata")) {
  ...
  # trim RTs>10000 ms
  data_trimmed <- data_for_analysis %>%
    filter(rt <= 10000)
  ...
  data_estimates_with_CIs <- data_D_scores %>%
    ...
    select(unique_id, domain, trial_type, D, D_ci_lower, D_ci_upper, D_sig, D_ci_width)
...
p2 <- 
  ...
  ggplot() +
  geom_linerange(aes(x = std_ordered_id, ymin = D_ci_lower, ymax = D_ci_upper, color = D_sig),
                 alpha = 1) +
