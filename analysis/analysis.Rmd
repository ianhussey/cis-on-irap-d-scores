...
- check all labelling/ordering in plots
...
p_ci_widths <- 
  ...
  geom_line() +
  geom_point() +
  scale_shape_manual(labels = c("BCA", "Basic", "Normal", "Percentile"), values = c(19, 15, 16, 17, 7)) +
...
results_diff_zero_method <- results_emm_diff_zero_method %>%
  mutate(cr_lower = estimate - (1.96 * sqrt(se^2 + sum(results_re_tau_diff_zero_method$tau)^2)),  # as in metafor package's implementation of credibility intervals, see metafor::predict.rma.R 
         cr_upper = estimate + (1.96 * sqrt(se^2 + sum(results_re_tau_diff_zero_method$tau)^2)))
...
# # summary
# summary(fit_diff_zero_scoring,  correlation = FALSE)
# 
# # plot RE effects
...
results_diff_zero_scoring <- results_emm_diff_zero_scoring %>%
  mutate(cr_lower = estimate - (1.96 * sqrt(se^2 + sum(results_re_tau_diff_zero_scoring$tau)^2)),  # as in metafor package's implementation of prediction intervals, see metafor::predict.rma.R 
         cr_upper = estimate + (1.96 * sqrt(se^2 + sum(results_re_tau_diff_zero_scoring$tau)^2)))
...
results_disciminability_scoring <- results_emm_disciminability_scoring %>%
  mutate(cr_lower = estimate - (1.96 * sqrt(se^2 + sum(results_re_tau_disciminability_scoring$tau)^2)),  # as in metafor package's implementation of credibility intervals, see metafor::predict.rma.R
         cr_upper = estimate + (1.96 * sqrt(se^2 + sum(results_re_tau_disciminability_scoring$tau)^2)))
...
results_ci_width_proportions_method <- results_emm_ci_width_proportions_method %>%
  mutate(cr_lower = estimate - (1.96 * sqrt(se^2 + sum(results_re_tau_ci_width_proportions_method$tau)^2)),  # as in metafor package's implementation of credibility intervals, see metafor::predict.rma.R 
         cr_upper = estimate + (1.96 * sqrt(se^2 + sum(results_re_tau_ci_width_proportions_method$tau)^2))) 

# plot
p_ci_width_proportion_observed_range_method <- 
  ggplot(results_ci_width_proportions_method, aes(method, estimate, 
                                                  color = method, 
                                                  shape = method, 
                                                  group = method)) +
  geom_linerange(aes(ymin = cr_lower, ymax = cr_upper), position = position_dodge(width = dodge_width), linetype = "dotted") +
...
results_ci_width_proportions_scoring <- results_emm_ci_width_proportions_scoring %>%
  mutate(cr_lower = estimate - (1.96 * sqrt(se^2 + sum(results_re_tau_ci_width_proportions_scoring$tau)^2)),  # as in metafor package's implementation of credibility intervals, see metafor::predict.rma.R
         cr_upper = estimate + (1.96 * sqrt(se^2 + sum(results_re_tau_ci_width_proportions_scoring$tau)^2)))
