...
# TODO example participant. median participant by d score in a domain with a high percent of sig di scores

```{r}

data_eg_1 <- data_trimmed %>%
  filter(unique_id == "Disgust (2)36.1" & trial_type == "tt1") 

data_eg_1_D <- 
  data_estimates_D %>%
    filter(unique_id == "Disgust (2)36.1" & trial_type == "tt1" & method == "bca")
...
# library(pwr)
# 
# pwr.t.test(n = 18, d = 0.4, type = "two.sample", sig.level = 0.05)
# 
# pwr.t.test(n = 18*6, d = 0.4, type = "two.sample", sig.level = 0.05)
# 
# library(esci)
# fit <- esci::estimateStandardizedMeanDifference(m1 = 1566, 
#                                                 m2 = 1402, 
#                                                 s1 = 527, 
#                                                 s2 = 452, 
#                                                 n1 = 18*10, 
...
if(file.exists("models/data_estimates_D_arithmetic.rds")) {
  
  data_estimates_D_arithmetic <- 
    read_rds("models/data_estimates_D_arithmetic.rds")
  
} else {
  
  arithmetic_D_score <- function(data){
    fit <- t.test(rt ~ block_type, data = data, var.equal = FALSE)
    
    rt_mean_diff <- as.numeric(fit$estimate[1] - fit$estimate[2])
    # rt_sem       <- fit$stderr
    d            <- rt_mean_diff/sd(dat$rt)
    d_ci_lower   <- fit$conf.int[1]/sd(dat$rt)
    d_ci_upper   <- fit$conf.int[2]/sd(dat$rt)
    
    return(data.frame(estimate = d,
                      ci_lower = d_ci_lower,
                      ci_upper = d_ci_upper))  
  }
  
  # bootstrap D scores 
  data_estimates_D_arithmetic <- data_trimmed %>%
    select(unique_id, domain, trial_type, rt, block_type) %>%
    group_by(unique_id, domain, trial_type) %>%
    do(arithmetic_D_score(data = .)) %>%
    ungroup() %>%
  ...
  # save to disk
  write_rds(data_estimates_D_arithmetic, 
            "models/data_estimates_D_arithmetic.rds", compress = "gz")
...
if(file.exists("models/data_estimates_D.rds")) {
  ...
  data_estimates_D_boot <- read_rds("models/data_estimates_D_boot.rds")
  ...
  # bootstrap D scores 
  data_estimates_D_boot <- data_trimmed %>%
    select(unique_id, domain, trial_type, rt, block_type) %>%
    group_by(unique_id, domain, trial_type) %>%
  ...
  # save to disk
  write_rds(data_estimates_D_boot, "models/data_estimates_D_boot.rds", compress = "gz")
...
data_estimates_D <- 
  bind_rows(data_estimates_D_arithmetic,
            data_estimates_D_boot)
...
data_estimates_D %>%
  arrange(estimate) %>%
  mutate(method = fct_relevel(method, "arithmetic", "bca", "basic", "normal", "percent")) %>%
  group_by(method) %>%
...
p_ci_widths <- 
  ...
  geom_line() +
  geom_point() +
  scale_shape_manual(labels = c("Arithmetic", "BCA", "Basic", "Normal", "Percentile"), values = c(19, 15, 16, 17, 7)) +
  scale_color_viridis_d(begin = 0.2, end = 0.8, labels = c("Arithmetic", "BCA", "Basic", "Normal", "Percentile")) +
  mdthemes::md_theme_linedraw() +
  facet_wrap(~ trial_type, ncol = 4, nrow = 1) +
  labs(x = "MAP 95% CI width",
       y = "",
       color = "CI estimation method",
       shape = "CI estimation method") + 
...
ggplot(data_ci_width_map_D, aes(MAP, method, color = trial_type, shape = trial_type)) +
  geom_jitter(width = 0, height = 0.25) +
  scale_shape_manual(values = c(15, 16, 17, 7)) +
  scale_color_viridis_d(begin = 0.2, end = 0.8) +
  mdthemes::md_theme_linedraw() +
...
data_diff_zero <- 
  ...
  mutate(domain = as.factor(domain),
         method = fct_relevel(method, "arithmetic", "bca", "basic", "normal", "percent"),
         method = fct_rev(method),
         trial_type = case_when(trial_type == "tt1" ~ "Trial type 1",
                                trial_type == "tt2" ~ "Trial type 2",
                                trial_type == "tt3" ~ "Trial type 3",
...
data_diff_zero %>%
  #filter(method == "bca") %>%
  filter(method %in% c("arithmetic", "bca")) %>%
...
fit_diff_zero_scoring <- 
  ...
  # filter(method == "bca") %>%
  filter(method %in% c("arithmetic", "bca")) %>%
  mutate(method = paste(method, DV_type)) %>%
  glmer(proportion_diff_zero ~ 1 + method + (1 | domain),
       weights = 1/variance, 
       family = "binomial",
       data = .)
...
if(file.exists("models/data_discriminability_D.rds")) {
  ...
  data_discriminability_D <- data_estimates_D %>%
    ...
    mutate(di_lower = estimate + (ci_lower - estimate)*sqrt(2),
...
fit_disciminability_scoring <- 
  # glmer(proportion_discriminable ~ 1 + trial_type + (1 | domain), # * DV_type
  #       weights = 1/variance, 
  #       family = "binomial",
  #       data = data_discriminability_combined_scoring)
  lmer(proportion_discriminable ~ 1 + DV_type + (1 | domain), # * DV_type
       weights = 1/variance, 
       data = data_discriminability_combined_scoring)
...
#   summary(emmeans(fit_disciminability_scoring, ~ 1 | domain)) %>%
#   dplyr::select(DV_type, trial_type, estimate = emmean, se = SE, ci_lower = lower.CL, ci_upper = upper.CL) %>%
#   mutate(trial_type = fct_relevel(trial_type, "Trial type 1", "Trial type 2", "Trial type 3", "Trial type 4"))
# 
# # combine
...
# 
# # tests
# emmeans(fit_disciminability_scoring, list(pairwise ~ 1 | trial_type), adjust = "bonferroni")
# 
...
fit_ci_width_proportions_scoring <- 
  ...
  #      data = .)
  lmer(ci_width_proportion ~ 1 + DV_type + (1 | domain) + (1 | unique_id), 
       data = .)
