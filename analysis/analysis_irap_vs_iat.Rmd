...
subtitle: "Comparing IRAP *D* scoring vs IAT *D scoring"
author: "Ian Hussey"
...
data_diff_zero <- 
  bind_rows(mutate(data_estimates_D, measure = "IRAP"),
...
p_diff_zero <- 
  ...
  ggplot(aes(proportion_diff_zero, domain, color = measure, shape = measure)) +
...
data_ci_width_proportions_iat_D <- data_estimates_iat_D %>%
  # join this data into the original data
  full_join(observed_range_estimates_iat_D, by = c("domain", "trial_type")) %>%
  # calculate ci width as a proportion of observed range
  mutate(ci_width_proportion = ci_width / range) %>%
  mutate(measure = "IAT")
...
ggsave(filename  = "plots/figure_1_cis_by_domain_irap_iat.pdf",
...
ggsave(filename  = "plots/figure_2_ci_widths_irap_iat.pdf",
       plot      = p_ci_widths,
       device    = "pdf",
       # path      = NULL,
       # dpi       = 300,
...
ggsave(filename  = "plots/figure_3_metaanalyses_irap_iat.pdf",
       plot      = p_combined,
       device    = "pdf",
       # path      = NULL,
       # dpi       = 300,
