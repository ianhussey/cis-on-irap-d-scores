...
simiulate_data_from_simulated_parameters <- function(data, n_simulated_trials) {
  data %>%
    ...
    # trim any simulated participants with implausibly short simulated RTs.
    # only .5% of RTs in the real data are less than 750 MS, so ditch these.
    # can't trim individual negative values or the data wont match the parameters, so ditch them wholesale
    group_by(id) %>%
    mutate(exclude = ifelse(min(simulated_value > 750), TRUE, FALSE)) %>%
    filter(exclude == TRUE) %>%
    ungroup() %>%
    dplyr::select(-exclude)
}

simulated_data_18 <- simiulate_data_from_simulated_parameters(data = data_sim_level_1_tidied, 
                                                              n_simulated_trials = 18)

simulated_data_180 <- simiulate_data_from_simulated_parameters(data = data_sim_level_1_tidied, 
                                                               n_simulated_trials = 180)
...
ggplot(simulated_data_18_summary, aes(sim_mean, mean)) +
  geom_point() +
  facet_wrap(~ block_type)

ggplot(simulated_data_18_summary, aes(sim_sd, sd)) +
  geom_point() +
  facet_wrap(~ block_type)
