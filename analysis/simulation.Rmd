...
data_sim_level_1 <- 
  MASS::mvrnorm(n = 1000, 
                mu = colMeans(dplyr::select(data_param, mean_con, mean_incon, log_sd_con, log_sd_incon)), 
                Sigma = as.matrix(cov(dplyr::select(data_param, mean_con, mean_incon, log_sd_con, log_sd_incon))), 
                empirical = TRUE) %>%
  as.data.frame() %>%
...
results <- rnorm_empirical(n = 18, mean = 1395, sd = 567)
mean(results$simulated_value)
sd(results$simulated_value)


data_sim_level_2 <- data_sim_level_1 %>%
...
temp <- data_sim_level_2 %>%
  #group_by(id) %>%
  select(id, mean_con, sd_con) %>%
  #nest() %>%
  #mutate(sim_rts = map(data, function(df) lm(mpg ~ wt, data = df)))
  mutate(sim_rts = map(rnorm_empirical(n = 18, mean = mean_con, sd = sd_con))) 
...
1:max(data_sim_level_2) %>%
  walk(rnorm_empirical(n = 18, mean = mean_con, sd = sd_con))



rnorm_empirical_helper <- function(id, n, mean, sd) { 
  tibble(id = id,
         mean = mean,
         sd = sd,
         n = n,
         simulated_value = mean + sd * scale(rnorm(n)))
}
...
temp <- data_sim_level_2 %>%
  pmap_dfr(function(...) {
    ...
    rnorm_empirical_helper(id = current$id, mean = current$mean_con, sd = current$sd_con, n = 18)
    # # # return
    # current %>
...
ggplot(temp2, aes(sim_mean, orig_mean)) +
  geom_point()

ggplot(temp2, aes(sim_sd, orig_sd)) +
  geom_point()
